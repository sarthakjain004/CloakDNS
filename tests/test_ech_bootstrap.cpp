...
void push_u16_be(vector<byte>& v, uint16_t x) {
    v.push_back(byte{static_cast<uint8_t>((x >> 8) & 0xff)});
    v.push_back(byte{static_cast<uint8_t>(x & 0xff)});
}

void push_byte(vector<byte>& v, uint8_t x) {
    v.push_back(byte{x});
}

void push_param(vector<byte>& v, uint16_t key,
                std::initializer_list<uint8_t> value) {
    push_u16_be(v, key);
    push_u16_be(v, static_cast<uint16_t>(value.size()));
...
TEST(SvcbExtractEch, ExtractsKeyFiveValue) {
    ...
    EXPECT_EQ(to_integer<uint8_t>((*out)[0]), 0xfeu);
    EXPECT_EQ(to_integer<uint8_t>((*out)[1]), 0x0du);
    EXPECT_EQ(to_integer<uint8_t>((*out)[2]), 0x42u);
    EXPECT_EQ(to_integer<uint8_t>((*out)[3]), 0xabu);
}
...
TEST(SvcbExtractEch, ExtractsWithSimpleTargetName) {
    ...
    // "example"
    push_byte(rdata, 7);
    for (char c : string{"example"}) push_byte(rdata, static_cast<uint8_t>(c));
    // root terminator
    push_byte(rdata, 0);
    ...
    EXPECT_EQ(to_integer<uint8_t>((*out)[0]), 0x99u);
}
